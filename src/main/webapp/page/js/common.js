...
var showDialog_newST = null;
function showDialog_new(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	clearTimeout(showDialog_newST);
	...
	if(locationtime) {
		...
		showDialog_newST = setTimeout(closefunc, locationtime * 1000);
		showconfirm = 0;
	}
...
function showDialog_new_new(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	clearTimeout(showDialog_newST);
	cover = isUndefined(cover) ? (mode == 'info' ? 0 : 1) : cover;
	leftmsg = isUndefined(leftmsg) ? '' : leftmsg;
	mode = in_array(mode, ['confirm', 'notice', 'info', 'right']) ? mode : 'alert';
...
function showWindow(k, url, mode, cache, menuv) {
	...
	var fetchContent = function() {
		...
		if(parseInt(BROWSER.ie) != 6) {
			loadingst = setTimeout(function() {showDialog_new('', 'info', '<img src="' + IMGDIR + '/loading.gif"> 请稍候...')}, 500);
		}
	};
...
function showError(msg) {
	...
	msg = msg.replace(p, '');
	if(msg !== '') {
		showDialog_new(msg, 'alert', '错误信息', null, true, null, '', '', '', 3);
	}
}
...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	if (!!("placeholder" in document.createElement("input"))) {
		if(searchtxt.value == '输入名称搜索') {
			searchtxt.value = '';
		}
		searchtxt.placeholder = '输入名称搜索';
	} else {
...
function addFavorite(url, title) {
	try {
		...
		try {
			...
			showDialog_new("请按 Ctrl+D 键添加到收藏夹", 'notice');
		}
	}
}
...
function setHomepage(sURL) {
	if(BROWSER.ie){
		...
		showDialog_new("非 IE 浏览器请手动将本站设为首页", 'notice');
		doane();
	}
}
